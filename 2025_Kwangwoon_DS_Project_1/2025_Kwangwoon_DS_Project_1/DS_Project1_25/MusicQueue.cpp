...
void MusicQueue::push(const string& artist, const string& title, const string& run_time) {
    if (size >= 100) {
        cout << "========ERROR========" << endl;
        cout << "200" << endl;
        cout << "====================" << endl;
    ...
    MusicQueueNode* newNode = new MusicQueueNode(artist, title, run_time);
...
MusicQueueNode* MusicQueue::front() const {
    if (empty()) {
        return nullptr;
    }
    return head;
}
int MusicQueue::getSize() const {
    return size;
}