...
// "분:초"를 초 단위로 변환
int PlayList::parseRuntime(const string& runtime) const {
...
void PlayList::insert_node(const string& artist, const string& title, const string& runtime) {
    if (full() || exist(artist, title)) {
        return;
    }

    int runtime_sec = parseRuntime(runtime);
